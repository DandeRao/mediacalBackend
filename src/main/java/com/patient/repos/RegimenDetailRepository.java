...
public interface RegimenDetailRepository extends JpaRepository< RegimenDetail, Long> {
    ...
    void deleteRegimenWithId(@Param("id") int id);
    ...
    @Query(value = "select * from regimen_detail where id in (select regimen_id from cancer_regimen_link where cancer_id in (select cancer_regimen_link.cancer_id from cancer_regimen_link where cancer_id =:cancerId)) ORDER BY disp_name ASC", nativeQuery = true)
